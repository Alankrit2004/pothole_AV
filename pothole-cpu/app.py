    ...
    def event_stream():
        ...
        while True:
            ...
            if job_snapshot["status"] in ("completed", "failed", "cancelled"):
                yield (
                    "event: stream_ended\n"
                    f"data: {json.dumps({'message': 'Alert stream stoped/cancelled/failed'})}\n\n"
                       )
                return
...
def get_output_image(filename):
    current_user = get_jwt_identity()
    safe_name = os.path.basename(filename)  # send_from_directory also guards traversal; belt & suspenders
    with OUTPUT_OWNERS_LOCK:
        owner = OUTPUT_OWNERS.get(safe_name)
    # Files predating this ownership tracking (or not in the map) fail
    # closed rather than being served to any authenticated user.
